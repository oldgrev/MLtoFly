...
class flyEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if(grounded or self.supercrash):
            ...
            if(self.altRad > 4.5 and self.altRad < 5.5):
                self.reward = 20 * self.actiondeltacoefficient * self.speedcoefficient
            else:
                self.reward = 0           
            if(abs(self.pitch) > 2 or abs(self.roll) > 2):
                self.reward = -1000                  
        ...
        if(self.altRad > 4.5 and self.altRad < 5.5):
            self.reward = (20 * self.actiondeltacoefficient * self.speedcoefficient) + 5
        else:
            self.reward = 0
        if(self.altRad < 4.5 and self.altRaddelta > 0.5):
            self.reward = 1
        if(self.altRad > 5.5):
            self.reward = 0
        if(self.egt > 0.625):
